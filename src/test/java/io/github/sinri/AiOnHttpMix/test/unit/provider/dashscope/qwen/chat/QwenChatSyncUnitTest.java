...
import io.github.sinri.AiOnHttpMix.provider.dashscope.qwen.request.parameters.QwenRequestSearchOptions;
 ...
 */
public class QwenChatSyncUnitTest extends AbstractQwenChatModelUnitTest {
    /**
     * 测试常规单轮对话。
     */
    @Test
    public void test1() {
        async(() -> {
            QwenRequest request = QwenRequest.create()
                                             .input(x -> x
                                                     .addMessage(
                                                             QwenMessageInChatRequest.createAsUserInRequest("以“东汉末年”开头写一部开高达征服世界的故事")
                                                     ))
                                             .parameters(x -> x
                                                     .maxTokens(1024));
    ...
    public void test2() {
        async(() -> {
            QwenRequest request = QwenRequest.create()
                                             .input(x -> x
                                                     .addMessage(QwenMessageInChatRequest.createAsSystemInRequest(
                                                             "你需要根据用户的输入作答，并将答案包装为一个 json object 输出，格式为 `{\"answer\":\"\"}`。"
                                                     ))
                                                     .addMessage(QwenMessageInChatRequest.createAsUserInRequest(
                                                             "2025年5月3日，杭州的天气是什么"
                                                     ))
                                             )
                                             .parameters(x -> x
    ...
    public void test3() {
        async(() -> {
            QwenRequest request = QwenRequest.create()
                                             .input(x -> x
                                                     .addMessage(
                                                             QwenMessageInChatRequest.createAsUserInRequest("2025年的梅雨时期是哪段时间？")
                                                     ))
                                             .parameters(x -> x
                                                     .maxTokens(1024)
                                                     .enableSearch(true)
    ...
    public void test4() {
        async(() -> {
            QwenRequest request = QwenRequest.create()
                                             .input(x -> x
                                                     .addMessage(
                                                             QwenMessageInChatRequest.createAsUserInRequest("夏天有什么解暑的东西推荐啊，已知西瓜吃吃太麻烦，雪糕最近又越来越贵……")
                                                     ))
                                             .parameters(x -> x
                                                     .enableThinking(true)
                                             );

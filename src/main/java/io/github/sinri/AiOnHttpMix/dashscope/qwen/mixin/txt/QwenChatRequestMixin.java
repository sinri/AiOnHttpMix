...
import io.github.sinri.AiOnHttpMix.dashscope.qwen.impl.request.InputImpl;
import io.github.sinri.AiOnHttpMix.dashscope.qwen.impl.request.ParametersImpl;
...
public interface QwenChatRequestMixin<E> extends JsonifiableEntity<E>, SelfInterface<E> {
    default E setModel(QwenKit.QwenModel model) {
    ...
    default E handleInput(Handler<Input> handler) {
    ...
    default E handleParameters(Handler<Parameters> handler) {
    ...
    interface Input extends JsonifiableEntity<Input> {
        ...
        static Input wrap(JsonObject jsonObject) {
            return new InputImpl(jsonObject);
        }

        Input addMessage(QwenKit.Message message);

        default Input addSystemMessage(String content) {
            return addMessage(QwenKit.Message.create()
                    .setRole(QwenMessageMixin.Role.system)
                    .setContent(content)
            );
        }
        ...
        default Input addUserMessage(String content) {
            return addMessage(QwenKit.Message.create()
                    .setRole(QwenMessageMixin.Role.user)
                    .setContent(content)
            );
        }
    }
    ...
    interface Parameters extends JsonifiableEntity<Parameters> {
        ...
        static Parameters wrap(JsonObject jsonObject) {
            return new ParametersImpl(jsonObject);
        }
         ...
         */
        default Parameters addTool(QwenKit.ToolDefinition toolDefinition) {
